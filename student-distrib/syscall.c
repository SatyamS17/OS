...
int32_t execute(const uint8_t* command) {
    ...
    if((oldPID == curr_pcb->pid)||(oldPID == -1)){
        printf("ALL PIDS USED");
        return -1;
    }else{
        curr_pcb = (pcb_t*) (KERNEL_END - (EIGHTKB_SIZE * (PID + 1)));
        curr_pcb->parent_pid = oldPID;
        curr_pcb->pid = PID;
        curr_pcb->ebp = (USER_ADDRESS + FOURMB_SIZE);
        curr_pcb->esp = (USER_ADDRESS + FOURMB_SIZE);
    }

    // -------- Setup Paging -------- //

    // make virtual mem map to right physical address
    uint32_t physical_address = KERNEL_END + (curr_pcb->pid * FOURMB_SIZE);
    ...
    uint32_t process_kernel_address = KERNEL_END - (curr_pcb->pid * EIGHTKB_SIZE) - 4;
    tss.ss0 = KERNEL_DS;
