...
typedef struct dentry_t
{
    ...
    uint32_t file_type;
    uint32_t inode;
    uint32_t reserved[DIR_ENTRY_RESERVED];
} dentry_t;
...
uint32_t file_write (uint32_t fd, const void* buf, uint32_t nbytes);
...
uint32_t dir_write (uint32_t fd, const void* buf, uint32_t nbytes);
